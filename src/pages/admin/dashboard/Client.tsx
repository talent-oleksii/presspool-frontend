        ...
        <div className='mt-4'>
          ...
          <div className='flex items-center grid grid-cols-4 gap-4'>
            <div className="col-span-1 rounded-[10px] pt-4 px-6 pb-2 bg-white">
              <div className='text-primary -tracking-[.48px] font-medium text-xs flex items-center'>
          ...
          <div className='flex items-center mt-4'>
            <div className='rounded-[10px] flex-1 border-[1px] border-[#7f8182] py-2 px-4 bg-[#edecf2] flex items-center'>
            ...
            {/* <select className='py-1.5 text-xs rounded-[10px] ms-4 border-[#7f8182] border-[1px] w-[23.5%] 2xl:w-[190px]'>
              <option>All</option>
            </select> */}
          </div>
          {/* <div className='mt-4'>
            <button
              className={`bg-main font-[Inter] font-semibold text-xs px-6 py-2 rounded-[10px] ring-[#7f8182] ${currentTab === 'active' ? 'ring-[1px]' : 'ring-0'}`}
              onClick={() => setCurrentTab('active')}
            >
              Active Clients
            </button>
            <button
              className={`bg-[#ffc3c0] font-[Inter] font-semibold text-xs px-6 py-2 rounded-[10px] ms-4 ring-[#7f8182] ${currentTab === 'inactive' ? 'ring-[1px]' : 'ring-0'}`}
          ...
          <div className='mt-4'>
            {
              showData && showData.map((item, index) => (
                <div key={item.id} className={`bg-white mb-2 px-[20px] rounded-[10px] relative flex py-4 items-center justify-between ${index !== showData.length - 1 && 'border-b-[1px] border-[#d9d9d9]'}`}>
                  <div className='text-left min-w-[170px] pr-2'>
                    <div className='mt-1 flex items-center'>
                      ...
                      <div className='ms-2'>
                        <p className='text-secondry1 text-xs font-[Inter] -tracking-[.36px]'>{item.company}</p>
                  ...
                  <div>
                    ...
                    <p className='font-[Inter] text-secondry1 text-xs font-medium -tracking-[.36px] mt-4'>{moment(Number(item.create_time)).format('DD/MM/YYYY')}</p>
                  </div>
                  <div>
                    <p className='font-[Inter] text-[#a3a3a3] text-xs font-medium -tracking-[.36px]'>Users</p>
                    <p className='font-[Inter] text-secondry1 text-xs font-medium -tracking-[.36px] mt-4'>{1}</p>
                  </div>
                  <div>
                    <p className='font-[Inter] text-[#a3a3a3] text-xs font-medium -tracking-[.36px]'>Total Campaigns</p>
                    <p className='font-[Inter] text-secondry1 text-xs font-medium -tracking-[.36px] mt-4'>{item.campaign_count}</p>
                  </div>
                  <div>
                    <p className='font-[Inter] text-[#a3a3a3] text-xs font-medium -tracking-[.36px]'>Total Spend</p>
                    <p className='font-[Inter] text-secondry1 text-xs font-medium -tracking-[.36px] mt-4'>{`$${item.spent}`}</p>
                  </div>
                  <div>
                    <p className='font-[Inter] text-[#a3a3a3] text-xs font-medium -tracking-[.36px]'>Account Manager</p>
                    <p className='font-[Inter] text-secondry1 text-xs font-medium -tracking-[.36px] mt-4'>{adminName}</p>
                  </div>
                  <div className='flex flex-col items-center justify-center'>
                    <p className='font-[Inter] text-[#a3a3a3] text-xs font-medium -tracking-[.36px]'>Status</p>
                    <p className={`font-[Inter] text-primary text-xs font-medium -tracking-[.36px] mt-4 px-2 rounded-[10px] py-1 ${item.state === 'active' ? 'bg-main' : 'bg-[#FF4D42]'}`}>{item.state === 'active' ? 'Active' : 'Inactive'}</p>
                  </div>
                  <div className='items-center flex justify-center'>
                    ...
                    <Link className='w-full h-full text-xs rounded-[10px] bg-black font-medium text-white px-4 py-2' to={`/admin/client/${item.id}`}>Client Details</Link>
                  </div>
