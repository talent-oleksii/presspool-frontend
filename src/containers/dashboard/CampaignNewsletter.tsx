  ...
  const data: Array<any> = useMemo(() => {
    const aggregatedData = newsletter.reduce((acc, entry) => {
      ...
      if (!acc[name].create_time) acc[name].create_time = entry.create_time;
        ...
        acc[name].create_time =
          acc[name].create_time > entry.create_time
            ? entry.create_time
            : acc[name].create_time;
      return acc;
    }, {});
    ...
    switch (filter) {
      case "name":
        ret.sort((a: any, b: any) => {
          return direction === "down" ? a.name - b.name : b.name - a.name;
        });
        break;
      case "total":
        ret.sort((a, b) => {
          return direction === "down"
            ? Number(a.total_clicks) - Number(b.total_clicks)
      ...
      case "unique":
        ret.sort((a, b) => {
          return direction === "down"
            ? Number(a.unique_clicks) - Number(b.unique_clicks)
            : Number(b.unique_clicks) - Number(a.unique_clicks);
        });
        break;
      // case 'verified':
      ...
      case "spend":
        ret.sort((a, b) => {
          return direction === "down"
            ? Number(a.verified_clicks) - Number(b.verified_clicks)
            : Number(b.verified_clicks) - Number(a.verified_clicks);
        });
        break;
    }
